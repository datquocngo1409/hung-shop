...
@toolbar-text: contrast(@toolbar-bg);
...
@switch-on-bg: @background;
@switch-on-text: darken(@background, 83%);
@switch-on-border: darken(@background, 14%);
@switch-on-gradient: null;
@switch-on-shadow: null;
@switch-on-focused-shadow: 0 0 3px 0 rgba(black, 0.3);
...
@adaptive-menu-title-text: @accent;
...
@adaptive-scheduler-base-text: @header-text-color;
...
@checkbox-checked-border: @checkbox-checked-text;
...
@checkbox-focused-border: contrast(@checkbox-bg, darken(@checkbox-border, 40%), lighten(@checkbox-border, 40%));
...
// Timepicker header
.k-time-header {
    color: lighten(@component-text, 30%);
